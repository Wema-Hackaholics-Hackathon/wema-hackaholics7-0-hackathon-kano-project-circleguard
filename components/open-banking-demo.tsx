...
export function OpenBankingDemo({ circleId, contributionAmount, nextPath }: { circleId?: string; contributionAmount: number; nextPath?: string }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-[#e1e5e2] bg-white">
    ...
    <div className="p-6">
      ...
      <button type="button" onClick={step === "select" ? () => selected ? setStep("consent") : setError("Choose a demo account first.") : step === "consent" ? connectAndAnalyze : () => setStep("result")} disabled={loading} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-[#123f31] px-5 py-3.5 font-semibold text-white disabled:opacity-60">{loading && <LoaderCircle size={17} className="animate-spin" />}{loading ? "Connecting…" : step === "select" ? "Continue to consent" : step === "consent" ? "Approve and connect" : "View readiness analysis"}{!loading && <ArrowRight size={17} />}</button>
