...
import { createCircle } from "@/app/circles/new/actions";
...
export function NewCircleForm() {
  ...
  const [payoutOrder, setPayoutOrder] = useState<"fixed" | "decide_later">("decide_later");
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.45fr_.8fr] lg:items-start">
          <form action={createCircle} className="overflow-hidden rounded-2xl border border-[#e1e5e2] bg-white">
            ...
            <div className="space-y-6 p-5 sm:p-7">
              <Field label="Circle name" hint="Choose a name members will recognise"><input name="name" className="input" placeholder="Enter circle name" value={name} onChange={(event) => setName(event.target.value)} minLength={2} required /></Field>
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <Field label="Contribution frequency"><select name="frequency" className="input" value={frequency} onChange={(event) => setFrequency(event.target.value as "weekly" | "monthly")}><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></Field>
              </div>
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <Field label="Start date"><div className="relative"><CalendarDays size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#7d8984]" /><input name="start_date" className="input pl-11" type="date" required /></div></Field>
              </div>
          ...
          <aside className="overflow-hidden rounded-2xl border border-[#dce3df] bg-white lg:sticky lg:top-8">
            <div className="border-b border-[#e7eae8] px-5 py-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#89948f]">Circle summary</p><h2 className="mt-2 text-lg font-semibold">{name.trim() || "Your circle"}</h2></div>
            <div className="space-y-4 p-5 text-sm"><Summary label="Each member pays" value={money.format(Number(amount) || 0)} /><Summary label="Members" value={members || "0"} /><Summary label="Frequency" value={frequency === "weekly" ? "Weekly" : "Monthly"} /><Summary label="Payout each cycle" value={money.format(payout)} strong /><Summary label="Estimated duration" value={`${members || "0"} ${frequency === "weekly" ? "weeks" : "months"}`} /></div>
...
function Choice({ title, copy, value, selected, onSelect }: { title: string; copy: string; value: "fixed" | "decide_later"; selected: boolean; onSelect: (value: "fixed" | "decide_later") => void }) { return <label className={`relative cursor-pointer rounded-xl border p-4 transition ${selected ? "border-[#2b7659] bg-[#f4f8f6]" : "border-[#e0e5e2] hover:border-[#b9c8c1]"}`}><input className="sr-only" type="radio" name="payout_order_method" value={value} checked={selected} onChange={() => onSelect(value)} /><p className="text-sm font-semibold">{title}</p><p className="mt-1 text-xs text-[#7c8782]">{copy}</p>{selected && <span className="absolute right-3 top-3 grid size-5 place-items-center rounded-full bg-[#2b7659] text-white"><Check size={12} /></span>}</label>; }
