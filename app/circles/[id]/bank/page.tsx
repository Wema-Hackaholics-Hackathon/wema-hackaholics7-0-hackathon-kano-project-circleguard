...
export default async function BankConnectionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!circle) notFound();
  return <AppShell active="Bank connections"><main className="p-5 sm:p-7 xl:p-10"><div className="mx-auto max-w-3xl"><Link href={`/circles/${id}`} className="inline-flex items-center gap-2 text-sm font-medium text-[#66736d]"><ArrowLeft size={16} /> Back to {circle.name}</Link><div className="mt-6"><p className="text-sm font-semibold text-[#2b7659]">DEMO BANKING</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.03em]">Account connection</h1><p className="mt-2 text-sm leading-6 text-[#6f7b76]">Connect a fictional bank profile, then watch its readiness change across simulated contribution cycles.</p></div><div className="mt-8"><OpenBankingDemo circleId={id} contributionAmount={Number(circle.contribution_amount)} /></div></div></main></AppShell>;
}
