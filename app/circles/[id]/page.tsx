...
export default async function CirclePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ invite?: string; error?: string }> }) {
  ...
  if (membership && bankProfileKey) {
    await supabase.from("demo_bank_connections").upsert({
      circle_id: id,
      user_id: user!.id,
      profile_key: bankProfileKey,
  ...
  const [{ data: assessmentRows }, { data: contributionRows }] = cycleIds.length > 0
    ? await Promise.all([
        supabase.from("readiness_assessments").select("cycle_id,profile_id,score,readiness,inflow_trend,on_time_rate").in("cycle_id", cycleIds),
  ...
  return <AppShell active="My circles"><main className="p-5 sm:p-7 xl:p-10"><div className="mx-auto max-w-[1050px]">
    ...
    {circle.status === "active" && <CycleDashboard circleId={id} isAdmin={isAdmin} memberCount={activeMembers.length} cycles={(cycleRows ?? []) as Array<{ id: string; cycle_number: number; due_date: string }>} assessments={(assessmentRows ?? []) as Array<{ cycle_id: string; profile_id: string; score: number; readiness: "ready" | "protection_recommended" | "action_required"; inflow_trend: string; on_time_rate: number }>} contributions={(contributionRows ?? []) as Array<{ cycle_id: string; profile_id: string; outcome: "early" | "on_time" | "late" | "failed" }>} memberNames={memberNames} />}
